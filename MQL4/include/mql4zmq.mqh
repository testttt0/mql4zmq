...
void release_buffer(int data, int hint);
...
int zmq_term (int arg_context)
{
   return(mql4zmq_term(arg_context));
}

// Sockets.
int zmq_socket (int arg_context, int type)
{
   return(mql4zmq_socket(arg_context, type));
}
...
//else Print("mssgLen: "+mssgLen); 
   ...
   RtlMoveMemory(stringChar, recvPtr, mssgLen);
   stringChar[mssgLen] = 0;
